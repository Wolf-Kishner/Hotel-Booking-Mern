...
export const PhotosUploader = ({ addedPhotos, onChange }) => {
  ...
  function SelectAsMainPhoto(ev , filename) {
    ev.preventDefault();
    const addedPhotosUnSelected = addedPhotos.filter(photo=> photo!==filename);
    const newAddedPhotos = [ filename , ...addedPhotosUnSelected];
        ...
        {addedPhotos.length > 0 &&
          addedPhotos.map((link) => (
            <div className="h-32 flex relative" key={link}>
              <img
                clasName="rounded-2xl w-full object-cover "
                src={"https://hotel-booking-mern-3b69.onrender.com/uploads/" + link}
              />
              <button
